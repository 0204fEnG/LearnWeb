...
const SectionNavbar = ({sectionsName,onNavClick,scrollInstance}) => {
    const navs = sectionsName.map((sectionName, index) => (<li className={['section-navbar__nav', Math.abs(scrollInstance - index)<=0.5? 'section-navbar__nav--active' : ''].join(' ')} onClick={() => onNavClick(index)}>{sectionName}</li>))
